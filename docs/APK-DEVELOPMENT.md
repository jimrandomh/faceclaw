...
Start with the portable kit generated by `android-sdk/scripts/export-portable-kit.sh`.
Unpack it, keep `starter/` and `sdk-repository/` together, and copy the whole
directory to a temporary location before verifying portability:
...
Use `implementation("com.faceclaw:sdk:0.2.0")`. The SDK supports Android 24+
...
preserved but is marked incompatible. Republish with SDK 0.2.0 after updating
...
`onHostConnected`, `onHostDisconnected`, and `onHostEvent` are app callbacks.
Unknown callbacks are no-ops. Handle input as a user event and recheck current
...
It records the revision, Java/Node/Gradle/Android tool probes, host typecheck
and tests, generator checks, SDK unit/assemble/lint tasks, portable Maven
export, a starter build from a copy outside the checkout, and both unmodified
...
```sh
python3 android-sdk/scripts/verify-portable-kit.py /path/to/faceclaw-sdk-0.2.0
