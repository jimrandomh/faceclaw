export type MenuHighlightAnimation = { dx: number; dy: number; startedAt: number; token: number };
  ...
  paint(index: number, scroll: number, x: number, y: number, width: number, height: number, now: number): MenuHighlightAnimation | undefined {
    ...
    if (!previous || previous.index !== index || previous.scroll !== scroll ||
      ...
      if (previous && this.animation) {
        const t = Math.max(0, Math.min(1, (now - this.animation.startedAt) / 500));
      ...
      this.animation = eligible ? { dx: Math.round(fromX - x), dy: Math.round(fromY - y), startedAt: now, token: nextToken++ } : undefined;
      this.fromIndex = null;
      this.previous = { index, scroll, x, y, width, height };
    }
    return this.animation && now - this.animation.startedAt < 500 ? this.animation : undefined;
  }
}
