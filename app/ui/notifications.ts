...
export class SingleNotificationLayer implements Layer {
  ...
  onRemoved(): void { this.removed = true; this.confirmation = null; }
    ...
    if (item.kind === "back") {
      ...
      if (notification.key.startsWith("apk:") && item.action.acceptsText) {
        ...
        const visible = () => !this.removed && shell.isScreenOn() && shell.foregroundWindow()?.windowId === foreground &&
          readActiveNotifications(ALL_NOTIFICATIONS, true).some(current => current.key === notification.key && current.postTime === notification.postTime);
