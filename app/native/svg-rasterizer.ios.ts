...
export function rasterizeSvg(svg: string, size: number, strokeWidth: number) {
  ...
  // Store icons use currentColor without a surrounding document/theme.
  // Resolve it explicitly; SVGKit does not inherit a host CSS color here.
  const source = svg.replace(/\bcurrentColor\b/gi, '#ffffff').replace(/<svg\b([^>]*)>/, (_tag, attributes: string) => {
